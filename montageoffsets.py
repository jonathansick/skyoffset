...
def work_montage(band, levelOnly=True):
    ...
    if os.path.exists(hdrPath) is False:
        print "mMakeHdr"
        print montage.mMakeHdr(rawTblPath, hdrPath, north_aligned=True,system="EQUJ")
    print "mHdrCheck"
    ...
    if os.path.exists(diffsPath) is False:
        print "mOverlaps"
        print montage.mOverlaps(projTblPath, diffsPath, exact=True, debug_level=None,
              status_file=None)
    ...
    if os.path.exists(diffFitsPath) is False:
        #print "mDiffFitExec"
        #print montage.mDiffFitExec(diffsPath, diffFitsPath, diffDir, debug=True)
        print "mFitExec"
    ...
    mosaicName = "montage_%s_%s" % (skyType, band)
    mosaicDB = mosaicdb.MosaicDB(dbname="m31", cname="mosaics")
    mosaicDoc = mosaicDB.collection.find({"_id": mosaicName})
    if mosaicDoc is not None:
...
def copy_blocks(band, workDir):
    """Resamples blocks into the TAN system, compatible with Montage."""
    ...
    blockDocs = blockDB.find_blocks({"FILTER": band,
        "field": {"$exists":1, "$nin":["M31-44","M31-45","M31-46"],
            "$in": ALLFIELDS}
        })
    ...
    for blockName, doc in blockDocs.iteritems():
        ...
        weightPath = doc['weight_path']
        newImagePath = os.path.join(workDir, os.path.basename(imagePath))
        newWeightPath = os.path.join(workDir, os.path.basename(weightPath))
        if os.path.exists(newImagePath) is False:
            print imagePath, newImagePath
            shutil.copy(imagePath, newImagePath)
            #shutil.copy(weightPath, newWeightPath)
            blockDB.collection.update({"_id":doc["_id"]},
                    {"$set": {"montage_input_image_path": newImagePath,
                        "montage_input_weight_path": newWeightPath}})

def resample_blocks(band, workDir, pixScale=1.):
    ...
    for dbid, imagePath in zip(ids, imagePaths):
        ...
        resampledWeightPath = os.path.join(workDir, basename+".resamp.weight.fits")
